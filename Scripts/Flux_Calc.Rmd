...
#write_xlsx()(all_fluxes, path= "../Computed_Flux/all_fluxes.csv")

#write.xlsx(all_fluxes, path= "../Computed_Flux/all_fluxes.xlsx", sheetName = "1", rowNames = FALSE)
```
...
```{r}
ggplot(licor_annotated,
       aes(x=datetime, y=CH4))+
  geom_point()
# 15 plots, very nice
```
...
ggplot(flux_plot,
       aes(x=Date, y=CH4_flux_g_ha_day,color= Treatment))+
  geom_point()+
  geom_line()
  #geom_errorbar(aes(ymin=CH4_flux_g_ha_day-CH4_flux_g_ha_day_se, ymax=CH4_flux_g_ha_day+CH4_flux_g_ha_day_se))
```
```{r}
